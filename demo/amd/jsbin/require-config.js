requirejs.config( {

    paths: {
        'jquery': 'https://code.jquery.com/jquery-1.11.3',
        'underscore': 'https://cdnjs.cloudflare.com/ajax/libs/underscore.js/1.8.3/underscore',
        'backbone': 'https://cdnjs.cloudflare.com/ajax/libs/backbone.js/1.2.1/backbone',
        'backbone.select': 'https://cdn.rawgit.com/hashchange/backbone.select/1.5.0/dist/amd/backbone.select',
        'backbone.cycle': 'https://cdn.rawgit.com/hashchange/backbone.cycle/2.1.0/dist/amd/backbone.cycle'
    }

} );
